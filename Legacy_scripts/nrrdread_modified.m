...
fname = sprintf('%s/%s_SLICER_RESULTS/%s2.nrrd',str{1},str{2},str{2})
fid = fopen(fname, 'rb');
...
% %Vadim masking
% fname3 = sprintf('%s_MASK/%s-%s-DTI.img',str{1},str{2},str2{1}) 
% fid2 = fopen(fname3);
% 
% m = fread(fid2,'uint8','ieee-be'); fclose(fid2);
% m = reshape(m,256,256,q); 
% 
% for k=1:q
%         for i=1:256
%             for j=1:256
...
fname5=sprintf('%s/%s_SLICER_RESULTS/%s_corrected_unmasked.d',str{1},str{2},str{2})
...
if (vdim == 1)
          % -- write data -- %
          count = fwrite( fid, buffer, 'float');
          fprintf('in vdim=1');
else
          A = permute(buffer,[4,1,2,3]);
          count = fwrite( fid, buffer, 'float');
        end
