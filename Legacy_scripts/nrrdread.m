...
for str = {'Seniors/MD/MD','Seniors/AH/AH','Seniors/CW/CW','Seniors/YC/YC','Junior/AM/AM','Junior/HT/HT','Junior/MM/MM','Junior/YT/YT'; 'MD', 'AH', 'CW', 'YC','AM','HT', 'MM','YT'}
...
fname = sprintf('%s_SLICER_RESULTS/%s.nrrd',str{1},str{2}); 
fid = fopen(fname, 'rb');
...
%X = permute(X, [2 1 3]);%usha, error for transpose presumably from the
%tensor data

%usha writing the file 
C= single(X);


%usha changing the order of the tensor entries to accomodate DTIStudio
%order
for k=1:p
    for j=1:256
        for i=1:256
           ...
           t = C(:,i,j,k);
           buffer(i,j,k,:)=[t(1) t(4) t(6) t(2) t(3) t(5)];
          % buffer(i,j,k)=t(5);
        end
    end
end
...
buffer2(i,j,q+1-k,:)=buffer(i,j,p-k,:);
        end
    end
end
...
fname5=sprintf('%s_SLICER_RESULTS/%s_%s.d',str{1},str{2},str2{1});
...
if (vdim == 1)
          % -- write data -- %
          count = fwrite( fid, buffer2, 'float');
          fprintf('in vdim=1');
else
          A = permute(buffer2,[4,1,2,3]);
          count = fwrite( fid, buffer2, 'float');
        end
    
	if count < 256*256*q*6
